...
esp_err_t mcp23017_deinit()
{
    ...
    // 1. Put all pins to input (Hi-Z)
    ret = register_write_byte(MCP23017_IODIRA, 0xFF);
    if (ret != ESP_OK) {ESP_LOGW(TAG, "BANK A NOT DEINIT");}

    ret = register_write_byte(MCP23017_IODIRB, 0xFF);
    if (ret != ESP_OK) {ESP_LOGW(TAG, "BANK B NOT DEINIT");}

    ret = i2c_master_bus_rm_device(s_i2c_dev_handle);
    ret = i2c_del_master_bus(s_i2c_bus_handle);    
    if (ret != ESP_OK) {ESP_LOGW(TAG, "I2C NOT DEINIT");}
